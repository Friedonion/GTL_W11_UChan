...
void ParticleSystemEmittersPanel::RenderEmitters(UParticleSystem* ParticleSystem)
{
    ...
    // 스타일 설정
    constexpr float EmitterHeaderHeight = 60.0f;
    ...
    {
        // 새 에미터 추가 버튼
        if (ImGui::Button("Add Emitter", ImVec2(120, 0)))
        {
            // 새 에미터 추가 로직 구현
        }
        ...
        if (ImGui::Button("Simulate", ImVec2(120, 0)))
        {
            // 시뮬레이션 로직 구현
        }
    }
...
void ParticleSystemEmittersPanel::RenderModules(UParticleEmitter* Emitter, int32 EmitterIndex)
{
    ...
    // 모듈 목록 영역
    ImGui::BeginChild(("ModulesRegion" + std::to_string(EmitterIndex)).c_str(), ImVec2(300.0f, 0), true);
    ImGui::Text("Modules:");
    ImGui::Separator();
...
void ParticleSystemEmittersPanel::RenderModuleCategory(const TArray<UParticleModule*>& Modules, int32 EmitterIndex)
{
    ...
    for (int32 ModuleIndex = 0; ModuleIndex < Modules.Num(); ++ModuleIndex)
    {
        ...
        ImGui::SameLine();
        {
            ...
            {
                if (ImGui::Button("Edit_Icon", ImVec2(30, 20)))
                {
